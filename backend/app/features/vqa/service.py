...
def answer_vqa_question(
    ...
    """Answer a VQA query for each retrieved keyframe using Multi-Candidate Generation and Consensus Judge."""
    if not top_kis_results:
        return [], 0.0
    ...
    is_color = is_color_question(question)
    is_pos = is_position_question(question)
    ...
    vlm_call_count = 0
    max_vlm_calls = getattr(settings, "VQA_MAX_CONCURRENCY", 8)

    for cand in top_kis_results:
        c = cand.copy()
        v_id = str(c.get("video_id", ""))
        f_id = int(c.get("frame_id", c.get("keyframe_id", 1)))
        
        try:
            meta_row = catalog.find_by_frame(v_id, f_id) if catalog and hasattr(catalog, "find_by_frame") else None
            kf_id = int(meta_row["keyframe_id"]) if meta_row and "keyframe_id" in meta_row else int(c.get("keyframe_id", f_id))
        except Exception:
            kf_id = int(c.get("keyframe_id", f_id))
        ...
        if hasattr(ocr_store, "get_frame_ocr_text"):
            text = ocr_store.get_frame_ocr_text(v_id, f_id) or ocr_store.get_frame_ocr_text(v_id, kf_id)
        ...
        if is_count and target_class:
            ...
            if not has_attr:
                ...
                conf = 0.92 if scale_res["status"] == "scale_aware_rcnn" else 0.80
        ...
        if is_exist and exist_target:
            exist_matches = [cls_name for cls_name in det_classes if exist_target in cls_name.lower()]
            candidates_pool.append(
                CandidateAnswer(
        ...
        if is_color and detections:
            ...
            if color_ans and color_ans != "unknown":
                candidates_pool.append(CandidateAnswer(text=color_ans, source="SYMBOLIC_HSV_CV", confidence=0.93, rationale=f"HSV color histogram classified as '{color_ans}'"))

        if is_pos and detections and not is_left_right:
            pos_ans = answer_symbolic_position_vqa([d["box"] for d in detections[:1]])
            if pos_ans:
                candidates_pool.append(CandidateAnswer(text=pos_ans, source="SYMBOLIC_CENTROID_CV", confidence=0.96, rationale=f"Centroid spatial coordinate placed at '{pos_ans}'"))

        if ocr_text and not is_left_right:
            ocr_patterns = ("text", "number", "word", "name", "written", "sign", "license", "plate", "chữ", "chu", "số", "so", "biển", "bien", "bảng", "bang", "tên", "ten", "thương hiệu", "thuong hieu", "hãng", "hang", "quán", "quan", "cửa hàng", "cua hang", "brand", "store", "shop", "hiệu", "hieu")
            ...
            if any(p in q_low or p in q_no_acc for p in ocr_patterns):
                candidates_pool.append(CandidateAnswer(text=ocr_text, source="OCR_TOKEN_RESOLVER", confidence=0.85 if has_visual_attributes(question) else 0.96, rationale=f"Extracted optical character recognition text '{ocr_text}'"))

        has_deterministic = any(cand.confidence >= 0.95 for cand in candidates_pool)
        should_run_vlm = not has_deterministic or is_left_right or (has_visual_attributes(question) and not any(cand.source == "SYMBOLIC_CENTROID_CV" for cand in candidates_pool))
        
        if (should_run_vlm or not candidates_pool) and vlm_call_count < max_vlm_calls:
            try:
                image_path = keyframe_image_path(v_id, kf_id, keyframes_dir=settings.KEYFRAMES_DIR)
                if not image_path.exists():
                    image_path = keyframe_image_path(v_id, f_id, keyframes_dir=settings.KEYFRAMES_DIR)
                if image_path.exists():
                    # Check visual cache natively inside engine
                    ...
                    if vlm_ans and vlm_ans.strip():
                        candidates_pool.append(CandidateAnswer(text=vlm_ans.strip()[:100], source="VLM_SPECULATIVE", confidence=0.88, rationale="Multimodal vision language model inference"))
                    vlm_call_count += 1
            except Exception as exc:
                logger.debug("VLM speculative inference skipped for cand %s: %s", v_id, exc)

        if not candidates_pool:
            candidates_pool.append(CandidateAnswer(text="", source="NO_ANSWER", confidence=0.0, rationale="No matching VQA engine"))

        winner, ranked_alternatives = consensus_judge.evaluate_and_pick(candidates_pool, detected_objects=det_classes, extracted_ocr_text=ocr_text)
